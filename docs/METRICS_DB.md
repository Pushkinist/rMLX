...
- **The `-dirty` suffix** is resolved **once at runtime**, at metrics init (inside `RunIdentity::get()`'s first call), via one `git status --porcelain` against that *same compile-time-stamped path* — never the cwd. Skipped entirely under `--metrics off`. Always absent (never "-dirty") when the compile-time path is empty, no longer exists, or is no longer a git repo — an installed binary has no source tree on the machine it runs on, so there is nothing that could be dirty.
