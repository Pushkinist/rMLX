...
| `--turbo-flash` | `on` \| `off` \| `auto` | `auto` | TurboFlash MSL attention kernel (K8V4 storage, `kv_seq > 4096`, `head_dim ∈ {128, 256}`). `auto` (default) resolves **OFF on every host**. The kernel decodes slower than the generic K8V4 path, and it can change the generated tokens, because it is the only `k8v4` configuration in which the 4-bit V codec runs at decode. `on` turns the kernel on. `off` is a hard override: an exported `RMLX_TURBO_FLASH=1` does not survive it. `auto` honours that variable and logs a `warn!` that the kernel is on. See `docs/KV_QUANT.md` § "TurboFlash is off by default". |
...
> yet). Layout detail in `docs/KV_QUANT.md` § "Iso memory truth".
