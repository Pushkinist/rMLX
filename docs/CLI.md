...
| `--rot-k-fused` | `on` \| `off` \| `auto` | `auto` | Fused FWHT + affine-quantize MSL kernel for the rot_k codec families (`--kv-quant rot_k`, `rot_k_tq4v`); every other codec ignores it. `auto` (default): OFF — the rotate-by-matmul path is the validated one; an exported `RMLX_ROT_K_FUSED=1` is honoured. `on` forces the fused kernel. `off` **hard-overrides** that variable. |
