...
| `--kv-quant` | string | `auto` | KV cache quantization preset: `auto` (unquantised bf16 on every arch and every context — see `docs/KV_QUANT.md` "The auto default"), `bf16` / `none` (unquantized), `k8v4`, `k8v8`, `planar`, `planar3` (3-bit V PlanarQuant), `k8vturbo3` (q8_0 K + TurboQuant 3-bit Lloyd-Max V; opt-in on every arch), `tsym4` (symmetric WHT-4 K + tq4 V; rejected on Qwen MoE arch with exit 78), `planar_k` (K-axis PlanarQuant 4-bit, V=bf16; rejected on Qwen MoE arch via `QwenMoePlanarKRejected`), `tsym3` (WHT-3 K + WHT-3 V symmetric; rejected on Qwen MoE arch). Mutually exclusive with `--cache-type-k`/`--cache-type-v`. |
...
| `--draft-block-size` | usize ≥ 2 | the drafter's declared depth, else 5 | Speculative round block: tokens the verifier scores per round, its own token included, so the drafter proposes one fewer — the same number for every drafter kind and the one `decode_config` records. **Absent, the round runs at the depth the drafter's own checkpoint declares** — the depth it was trained at — and at 5 only for a drafter that declares none (the Gemma4 assistant and the two-model arm). Refused below 2 at parse time. What bounds it above depends on the drafter: a DFlash checkpoint's own `block_size` caps it, because that block is the shape of the drafter's denoising input, while the Qwen3.5-family MTP sidecars do not cap it — all three shipped ones declare `block_size: 3` and that is the depth the head was trained at, not one it can only propose to, since it chains on its own output hidden. Every loop is bounded above by what one un-chunked verify forward can score. Env: `MLX_VLM_DRAFT_BLOCK_SIZE`. |
...
| `--paged-kv` | bool flag | off | Route K8V4/K8V8/Planar caches through the block-table paged storage path. Incompatible with `bf16`/`none` and `rot_k*` cache types. **Requires an explicit `--kv-quant`.** `--kv-quant auto` is unquantised bf16, which has no packed store to page, so `--paged-kv` alone is refused at startup (exit 1) with a message naming a codec to pass. This changed when `auto` became bf16; `--paged-kv` alone used to inherit a quantised per-arch default. The flag is not auto-promoted to a codec — that would be a second codec resolver keyed on something other than `--kv-quant`. |
...
> `rotor3_sym`, `rotor4_sym`, `k_rotor3`, `k_rotor4` all spend one whole `u32`
> code word **and** one scale per group — 4 head-dim slots for iso, 3 for
> rotor — so the 3-bit and 4-bit member of each family occupy byte-identical
> storage and the nominal width never reaches memory. With the scale and norm
> planes at `KV_SIDEBAND_DTYPE` (bf16) the ring holds **12.125 bits/value for
> iso and 16.25 for rotor** at `head_dim = 128`, against bf16's 16.0: iso
> saves 24%, rotor is still larger than the baseline, and no further sideband
> narrowing changes that — the `u32`-per-3-slots code cadence is what keeps
> rotor above. Both were 16.25 / 21.75 while those planes were `f32`. Pick a
> rotor codec for quality study or kernel work, not to shrink the cache. Note
> also that an `iso3` / `iso4` chosen with a q8 K builds **no packed store at
> all** — that pairing decodes from the bf16 mirror, so it neither costs nor
> saves; only the `*_sym` and `k_iso*` pairings put iso bytes on a ring.
> rMLX says so at resolve time: a `warn!` carrying the byte math fires
...
> 8-bit form when its widths are parameters and the stack keeps no bf16 K/V
> mirror for its layers to share), but only under a base mode that
...
| `--device` | `cpu \| gpu` | `gpu` | Inference device. |
| `--kv-quant` | string | `auto` | KV cache quantization preset. `auto` is unquantised bf16 on every arch and every context. |
| `--kv-preset` | string | — | Named KV-cache preset. Mutually exclusive with `--kv-quant`, `--cache-type-k`, `--cache-type-v`, `--kv-bits`. Values: `auto` (identical to `--kv-quant auto` — the same `DEFAULT_KV_QUANT`), `fp16`, `q8`, `speed`, `quality`, `planar`, `planar3`, `k_only_planar`. No non-`fp16` preset reduces resident KV — see `docs/KV_QUANT.md §Codec disposition`. |
...
| `MLX_VLM_DRAFT_BLOCK_SIZE` | `--draft-block-size` | the drafter's declared depth, else 5 | Speculative round block, verifier token included; ≥ 2. Unset, the drafter's declared depth decides. Flag wins. |
