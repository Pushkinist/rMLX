...
| 2 | Model + layers | `<arch>/model.rs`, `<arch>/layers.rs` (or a `<arch>/layers/` dir) | Arch-specific forward math. Attention goes through `KvCache::update_and_sdpa` (`rmlx-kv-quant`); build masks with `layers/mask.rs` (`build_chunked_prefill_mask`, `build_swa_prefill_mask`, `pick_attn_mask_mode`). The model struct also carries one shared field, `kv_bytes: KvBytesCounter`, initialised in the loader (#3), written at the end of generate (#4) and read through the `Architecture` dispatch (#6). Per instance, never a static — an arch-level static cross-attributes two resident models of the same arch. |
