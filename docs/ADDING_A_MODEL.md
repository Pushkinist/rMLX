...
| 6 | Enum variant | `arch/mod.rs` | One `Architecture` variant + the match arms (`forward_seq`, `Debug`, config summary, …). `model_sig()` is one of them: the SSD-tier attach reads the model's signature through it, and a variant that forgets the arm cannot compile. |
