...
| 4 | Generate | `<arch>/generate.rs` (or a `<arch>/generate/` dir) | Prompt-cache policy + cache construction + a `forward_step` closure handed to the shared `decode_loop` (`pipelined_decode` / `chunked_prefill` / `choose_token`). No decode-loop copy. Every prefill path — ordinary or speculative — calls `reject_nan_prefill` on its logit row **before** selecting a token. Per-*step* decode rows are deliberately not scanned (that would be a host readback per token); see the seam list below. |
...
- `decode_loop.rs` (`crates/rmlx-models/src/decode_loop.rs`):
  ...
  - `reject_nan_prefill(arch, dtype, nan_count, max_abs_logit, prompt_len)` — mandatory prefill guard. Call it on every prefill path (fresh, hydrated-tail, image) right after counting NaN in the logit row and **before** `choose_token`: greedy selection over an all-NaN row returns index 0 whatever the model computed, so emitting that token and returning `Ok` reports a fault as a one-token generation with exit 0. Raising before the token reaches `step_fn` is also what lets the server's retry envelope replay the request. Pass the row's real `dtype`: the NaN scan reads only F32 and Bf16 and reports `0` for anything else, so the guard refuses an unreadable row rather than treating "never checked" as "clean". `make check-no-decode-swallow` RULE 4 fails the build if a `count_nan_in_bytes` call site in the arch layer does not propagate, if the propagation lands after `step_fn`, or if the detector/guard call-site counts drift.
  - Speculative drivers use `speculative::guard_verifier_prefill_logits(verifier, logits, prompt_len)` at the same position — the one verifier logit row the driver argmaxes for its first bonus token.
