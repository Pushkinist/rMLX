...
* **`niah_long_context.rs` has no slug fallback, so every NIAH cell stands down
  ...
  just left. It is not fixed here because arming the resolution would change
  nothing that runs — but the reason NIAH does not run is a detector limitation,
  not a decision, and it is worth stating as such because it can expire:

  Every NIAH cell is generated by `macro_rules! niah_cell!` / `niah_pflash_cell!`
  (61 invocations; the file contains **zero** line-leading `#[test]`).
  `check_gpu_tests_ignored.sh` is a source scanner whose fn pattern matches
  `fn <ident>`, and a macro body's `fn $name()` does not match, so the cells are
  invisible to it — `bash scripts/check_gpu_tests_ignored.sh --list | grep -c
  niah` → `0`. `scripts/run_gpu_tests.sh` executes exactly what that classifier
  lists, so NIAH is unreached because **the detector is blind**, not because
  anyone scoped it out.

  The condition under which this justification expires: the day the classifier
  learns about macro-expanded tests, or someone unrolls the macro, NIAH joins
  `make gpu-test` carrying a resolver that stands down by default. Arm its
  resolution then, together with deciding which gate should own it.

  The same blindness is a live gate-integrity gap in its own right — see the
  note under the `#[ignore]` convention below.
