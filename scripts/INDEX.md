...
| `published_samples.py` | `verify`: the checked-in published-protocol sample sets (`prompts/published/`) agree with the facts pinned in this script — seed, revision, template, count, licence, and the digest and byte length of each built file and each upstream file — and, against the manifest, the draw re-derives from its seed over the recorded pool, each sample id from its source_id, each user message from the recorded template and the sample's own copy of the upstream record, and each body digest from the encoding of its messages. The anchor is deliberately in the script and not only in the manifest: a manifest beside the data can be re-blessed around an edited file in the same commit, a pin cannot without a diff. `--sources <dir>` holds the samples to the upstream files at the pinned revision, record for record; no gate passes it, because the upstream files are not checked in. `build` is the one producer of those files and prints a replacement pin block whenever what it produced moved. That the Python body digest is the id the Rust recorder gives the same body is checked separately, by `published_sample_body_digests_match_this_recorder` in `crates/rmlx-metrics/src/prompts_tests.rs`. |
| `check_published_samples_fixtures.sh` | Recall test for the above — 30 synthetic sample-set roots, one edit each. Both halves of the anchor: a rewritten prompt with the manifest re-blessed around it (caught by the script's pin) and the same edit with the pin moved too (caught, under `--sources`, against the upstream record). Then a flipped byte, a truncated file, a wrong seed, an edited template, revision and licence, a set emptied to nothing, an undrawn sample, a desynced sample id, a duplicated and a dropped one, a stale body digest, an edited prompt with every digest re-blessed, messages re-emitted in the wrong key order, an upstream file off its pinned revision, one that no longer parses, one whose records lost their id field, a reordered pool, a missing source, a sample that is not an object, and each way the manifest itself can be empty, short, unreadable, absent or of a schema this script does not read. A positive control pins that the record comparison passes on unedited data. Each asserts the reason as well as exit 1 vs exit 2. |
...
| `published_ingest_selftest.sh` | `make published-ingest-selftest` | Mutation check for `ingest/published_ingest.py` — 25 cases over one synthetic result on the REAL checked-in sample sets, one edit each, asserting the literal exit code and the reason. Both ways one sample's digest can move (against the manifest, and between the passes), a sample set edited after the run, a rebuilt binary, a binary whose digest still agrees but which lost a marker, a result with no binary identity at all, each gate and its waiver, and the queue discipline in all three outcomes — staged outside `pending/`, moved through it and back, and kept in staging when the recorder rejects it. Never writes `runs.db`. In `make ci`. |
...
| `spec_bench_published.sh` | — | **Bench one verifier under the published on-device speculative-decoding protocol**: one server per pass, one chat request per checked-in sample in `prompts/published/`, three passes. The headline is the macro average over the three datasets at the 1024-token budget — one cell per dataset, so MATH-500's second budget is a column beside it and not a fourth dataset; a dataset with no cell there, or two, is refused rather than dropped or counted twice. Every request sends the checkpoint's own sampling by carrying no sampling field, thinking on, and what that resolved to is read back from the engine's own event rather than re-derived from the checkpoint's file — the seed included, which the request does not send and the engine substitutes, so the three passes replay one RNG stream. Each decode rate is the engine's reading of the first-to-last-token window — the round loop's `done` line on the speculative arm, the per-request ITL aggregate on the plain one — cross-checked against the same window timed client-side; the two rings at `GET /metrics/cache` hold twenty entries and cannot carry a 128-sample dataset. A dataset mean whose run-to-run range exceeds 5% is withheld and named, and the macro is withheld with it. `prompts/published/` is held to `published_samples.py` with no way to skip it; `--samples-root` is an unverified operator override whose result says so. Before any server starts, the binary is identified by digest AND by the log-message literals the readings are read off, so a stale build in `target/` is caught before three passes are spent on it. The protocol's second figure — autoregressive output speed, input speed and resident memory on one prompt of a stated length — is measured on the plain arm only: the body is fitted to the target against this checkpoint on a preparation server, sent once per pass before the cells so its prefill is cold, and its resident figure is the peak of `phys_footprint` sampled at a recorded interval, so it reads as the lower bound a sampled gauge is. Thermal state is read three times a pass from `pmset -g therm` — `powermetrics` needs sudo and a non-interactive run does not have it — and a throttled or unreadable state joins the same taint the interference gate writes to. The seed is left at the engine's default in all three passes, so the run-to-run range is machine variance with the sampling held still; `divergent_samples` measures whether that held rather than asserting it. Same `--synthetic-arms` boundary as `perf_ab.sh`, from `lib/cpu_snapshot.sh`. Writes no `runs.db` row — `ingest/published_ingest.py` is the separate, explicit step. |
| `spec_bench_published_selftest.sh` | `make spec-bench-published-selftest` | Mutation check for the above against a stub server over a shrunken copy of the checked-in sample sets — 70 cases, each asserting a literal exit code and, for every refusal, the reason. The macro's weighting is staged so that averaging the cells, averaging the datasets across both budgets, and pooling the rows are three different wrong numbers; one dataset is destabilised on its own to show the others are not refused with it and the macro is withheld even though its own range is inside the band; the band is pinned just under, just over and exactly on. The published root is shown to be verified by reaching it as the published root, and the override is shown to skip that and mark itself. Every request's messages still hash to their checked-in content address, no sampling field is ever sent, the warmup's prompt is in no sample set, and the `--synthetic-arms` boundary is pinned in both directions from a `ps` shim. A binary that lost one marker literal is refused before it serves; a throttled and an unreadable thermometer each taint while a posted level at full speed does not, all three from a `pmset` shim so no verdict depends on this Mac's own thermal history; the fixed-length prompt lands on the target exactly or the run is refused, a memory gauge that is absent or reads zero is refused, and its spread is refused without refusing the cells. No GPU, no model, no DB. In `make ci`. |
| `published_table_selftest.sh` | `make published-table-selftest` | Mutation check for `lib/published_table.py` — 33 cases against a header-only fixture snapshot, each asserting a literal exit code and, for a refusal, the reason. The numeric cases do not check that the bound column is *present*: they re-derive the ceiling and the resident floor from the fixture's own safetensors header and `config.json`, in arithmetic sharing no code with `perf_ceiling.py`, and require the rendered figure to match inside the precision it is printed at. That is possible only because the fixture is chosen to make it possible — dense, no MoE, no sliding window, untied embeddings, `--kv-quant none` — so the codec and the context ceiling are instead checked to be *passed through*: change either and the rendered figure must move. `k8v8` is not the codec that case uses, because on this stack it decodes off its bf16 mirror and prices identically to `none`, so the test could never have failed. Also: a ceiling that moves across a decode window is refused, a snapshot or a host that is not the run's is refused, several results that are not one comparison are refused six ways, an unstable mean leaves no percentage behind it, a resized block quotes no block-kept fraction, and the fixture banner is shown to appear and to not be boilerplate. In `make ci`. |
...
| `perf_ceiling.py` | — | Static roofline calculator: bytes/step and the theoretical ceiling from a snapshot's `config.json` + safetensors index. Two weight censuses over the same headers, because they answer different questions: `weight_bytes_step` is what one decode step streams (MoE scaled to `top_k`, gathered embeddings excluded) and is the ceiling's numerator; `resident_weight_bytes` is what text decode must hold (every expert, the embedding table included, multimodal towers excluded) and with `kv_resident_bytes` is a resident-memory floor. Its KV byte model is a second copy of the engine's and is held to it by `check_kv_byte_model_parity.sh`; `--byte-model` is that gate's entry point. |
...
| `ingest/published_ingest.py` | Promote one `spec_bench_published.sh` result into §8.5 RunRecords: one per (cell, sample) carrying `decode_tps_warm` + `ttft_warm_ms` as the mean of the three passes, plus one for the fixed-length prompt carrying output speed, input speed, TTFT and both resident peaks. The per-dataset and macro averages are deliberately NOT recorded — they are means over those rows, and a stored derivation is a second copy that can disagree with what it was derived from. `prompt_id` is each sample's own body, and the digest recorded with the measurement, the digest in the sample manifest and the digest this recorder gives the body being submitted must be one value: a mismatch is a hard refusal, because a row attributed to a prompt that was never sent splits every later join with nothing saying so. Re-hashes the binary and re-scans it for the marker literals the run recorded, and re-runs `published_samples.py verify` on the sample root — both catch a rebuild between the measurement and the record. Refuses a `--synthetic-arms` run and an unverified sample root with no waiver at all, and a TAINTED run unless told otherwise, carrying the taint and the thermal readings into `notes` either way. Identity is the run's, stamped when it was made, never re-derived here. |
...
| `lib/binary_identity.py` | Which binary a run measured: its sha256 and the log-message literals the run's readers grep for, imported from those readers rather than restated. A digest alone does not separate a build from the stale one a stash-build-unstash cycle left in `target/` — both hash the same because both are one file. `release-perf` sets `strip = "symbols"`, so the message text is what survives to key on and the scan is over the file's bytes. |
| `lib/published_fixed_prompt.py` | Fit the published protocol's fixed-length prompt against a live server. The token count belongs to the tokenizer as much as to the bytes, and this repository ships no tokenizer, so the body is not checked in: it is derived from a checked-in corpus, a stated cut rule (the longest word-boundary prefix at or below the target, plus filler words whose cost is measured rather than assumed) and the server's own `usage.prompt_tokens`. A target the corpus cannot reach exactly is refused rather than rounded to. |
| `lib/published_fixed_run.py` | Assemble one pass's fixed-length-prompt run: the engine's decode window cross-checked against the client's, input speed as prompt tokens over TTFT on a cold prompt cache, and the peak the two process-memory gauges reached while the request was in flight. A run with no memory sample is refused — a peak over nothing is not a small peak — and the body travels with it, because the fitted prompt is checked in nowhere. |
| `lib/bench_server.sh` | Start, wait for and identify one rmlx server: preflight, readiness polling, the run-log-by-pid attribution and the resolved-codec reader. **Source it.** Shared by `spec_bench.sh` and `spec_bench_published.sh`. |
| `lib/published_table.py` | Render published-protocol results as the Markdown table a post needs — one result per engine mode, rows compared down a column only once the inputs are shown to be one comparison (same checkpoint, codec, context ceiling, protocol constants, cells and binary). **No measured figure is printed without the figure it cannot exceed**: the decode rate against `perf_ceiling.py`'s bandwidth-bound autoregressive ceiling at that cell's own codec and context, the resident peaks against the weights text decode must hold plus the KV the ring holds, and `tokens_per_round` against the block the drafter was given — left empty for a loop whose own `done` line says it resized that block. The ceiling is `perf_ceiling.py`'s and none is derived here; what that instrument cannot express is printed as absent rather than guessed at, which is why a speculative arm is priced against the *autoregressive* bound with the column saying so (over 100% is the point of speculative decoding) and why the input-speed bound is empty without a measured prefill anchor. A percentage of a bound renders just as plausibly when the bound is wrong, so the ceiling is refused rather than approximated when it moves across a cell's decode window by more than the band, when the snapshot is not the one measured, and when the host is not the one the bandwidth constant belongs to unless that host's bandwidth is stated. Prints the pinned protocol choices and the three disclosures — first-turn-only MT-Bench, the macro's one-cell-per-dataset definition, and the fixed seed with its measured `divergent_samples` check — in the header, not a footnote. |
...
| `lib/published_run_log.py` | Read one run log back as a per-request table — each request's TTFT, the engine's own decode rate, and the sampling it says it resolved, from the `generate_streaming: TTFT`, `generate: ITL stats` and `host categorical sampler active` events on the plain arm and the round-loop `done` line on the speculative one, zipped in file order. The `done` line's contract stays in `lib/spec_round_log.py`; this calls it. A request the engine could not time is refused, not dropped — a mean over the samples that worked is not a mean over the sample set — and requests of one pass that did not share a sampling setup are refused too. |
| `lib/published_aggregate.py` | `pass`: zip one pass's client-side blocks with the engine's rows, require the two readings of each decode window to agree, and take the completion count from the usage chunk rather than the reader's content-chunk fallback. `report`: three passes to per-dataset and macro means with their run-to-run range. The macro is one cell per dataset at `--macro-max-tokens`, never a mean over the cells and never over the pooled samples. A mean whose range exceeds the band is withheld from the mean column and named; the macro has no refusal of its own — its range is bounded by the widest dataset's — and is withheld whenever a dataset it covers is. `sample_range_pct_max` reports the widest single-sample spread, which the pass-mean range cannot see and which is never a refusal, and `divergent_samples` counts the samples that did not generate the same length in all three passes — the measured check on the claim that the range is machine variance. `--fixed` adds the fixed-length-prompt block: its rates are refused on the same band, and its resident figures are peaks, reported as the maximum over the three runs because a mean of three peaks is not a peak of anything. |
| `lib/published_payloads.py` | Render one request payload per checked-in sample and index them, plus the untimed warmup — whose prompt is in no sample set, so no measured sample faces a warm prompt cache while the others are cold. Refuses both ways the cell table and the sample root can drift apart: a cell naming a dataset the root lacks, and a dataset the root holds that no cell measures. The `messages` array is copied verbatim — key order is the content address — and no sampling field is emitted, because the published sampling is the checkpoint's own. |
...
| `lib/snapshot_sampling.py` | Pre-flight: the three sampling fields the published protocol names — temperature, top_p, top_k — must be stated by the checkpoint, because a run that sends none would otherwise be measured under the engine's fallbacks (1.0, 1.0, and top-k disabled) and those would be published as the checkpoint's own. `repetition_penalty` is deliberately not required: the protocol does not name it and its fallback is the neutral 1.0. Also says whether the checkpoint is greedy, which decides whether a resolved-sampler event exists to read back at all. |
