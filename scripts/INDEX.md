...
| `gpu_validation_census.txt` | — | Data, not a script: the shader-validation hits `run_gpu_tests.sh` accepts — kernel, access kind, exact count, and the analysis each rests on. Anything else is a failure naming the delta. |
| `run_gpu_tests_selftest.sh` | `make gpu-runner-selftest` | Recall test for the runner's reporting: a shader-validation hit and a crate failure in the same run are both reported, the access mix is the one observed, and every census-pin verdict fails (or passes) with its own reason. Stubbed crates, no GPU. |
