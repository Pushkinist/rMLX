...
| `spec_bench_published.sh` | — | **Bench one verifier under the published on-device speculative-decoding protocol**: one server per pass, one chat request per checked-in sample in `prompts/published/`, three passes, macro-averaged over MT-Bench / MATH-500 / HumanEval and reported per dataset. Every request sends the checkpoint's own sampling defaults by carrying no sampling field at all, thinking on, at 1024 output tokens and MATH-500 again at 4096. Each decode rate is the engine's own reading of the first-to-last-token window — the round loop's `done` line on the speculative arm, the per-request ITL aggregate on the plain one — cross-checked against the same window timed client-side; the two rings at `GET /metrics/cache` hold twenty entries and cannot carry a 128-sample dataset. A mean whose run-to-run range exceeds 5% is withheld and named rather than published. Same `--synthetic-arms` boundary as `perf_ab.sh`, from `lib/cpu_snapshot.sh`. Writes no `runs.db` row. |
| `spec_bench_published_selftest.sh` | `make spec-bench-published-selftest` | Mutation check for the above against a stub server over a shrunken copy of the checked-in sample sets — 42 cases, each asserting a literal exit code and, for every refusal, the reason. Both sides of the range band (4.9% publishes a mean, 5.12% does not), the macro average proven unweighted by staging datasets of 2 / 3 / 4 samples so a pooled mean would be a different number, every request's messages still hashing to their checked-in content address, no sampling field ever sent, and the `--synthetic-arms` boundary pinned in both directions. No GPU, no model, no DB. In `make ci`. |
...
| `lib/published_run_log.py` | Read one run log back as a per-request table — each request's TTFT and the engine's own decode rate, from the `generate_streaming: TTFT` and `generate: ITL stats` events on the plain arm and the round-loop `done` line on the speculative one, zipped in file order. The `done` line's contract stays in `lib/spec_round_log.py`; this calls it. A request the engine could not time is refused, not dropped — a mean over the samples that worked is not a mean over the sample set. |
| `lib/published_aggregate.py` | `pass`: zip one pass's client-side blocks with the engine's rows and require the two readings of each decode window to agree. `report`: three passes to per-dataset and macro means with their run-to-run range. The macro average is the mean of the dataset means, never of the pooled samples. A mean whose range exceeds the band is withheld from the mean column and named, and the report exits 3. |
| `lib/published_payloads.py` | Render one request payload per checked-in sample and index them, refusing both ways the cell table and the sample root can drift apart: a cell naming a dataset the root lacks, and a dataset the root holds that no cell measures. The `messages` array is copied verbatim — key order is the content address — and no sampling field is emitted, because the published sampling is the checkpoint's own. |
