...
| `check_kv_codec_disposition.sh` | Nine rules over the operator-facing KV help and every `docs/KV_QUANT.md` INERT banner, all derived from `ALL_KV_QUANTS` + the three decode predicates. Rejects any resident-KV ratio written into a help constant (one producer: `rmlx info --list-cache-types`), fails closed on a help constant nobody put in scope, and fails when the listing that help points at has no call site. |
| `check_kv_codec_disposition_fixtures.sh` | Recall test for the above — 17 synthetic scan roots (one edit each), asserting which rule fired and exit 2 vs exit 1. |
