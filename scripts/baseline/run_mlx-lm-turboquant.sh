#!/usr/bin/env bash
# Baseline measurement using the arozanov mlx-lm-turboquant fork.
#
# Usage:
#   ./run_mlx-lm-turboquant.sh <model_dir> <prompt_file> <max_tokens> [cpu|gpu]
#
# Appends one row to metrics/baseline.csv (cross-backend schema).
# Note: gemma4 arch is not supported by this fork — those runs produce
# an ARCH_NOT_SUPPORTED row automatically.
...
VENV_GENERATE="${MLX_LM_TURBOQUANT_ROOT:-../mlx-lm-turboquant}/.venv/bin/mlx_lm.generate"
...
run_id = datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S') + '-tq'
...
try:
    ...
    print(f'{backend}: {model_basename} TIMEOUT (300s)')
    sys.exit(0)

# Arch not supported — turboquant fork does not support gemma4
if 'not supported' in combined.lower():
    ...
    print(f'{backend}: {model_basename} ARCH_NOT_SUPPORTED')
    sys.exit(0)
...
print(f'{backend}: {model_basename}  load={load_ms}ms  ttft={ttft_ms}ms  tps={gen_tps:.3f}  rss={peak_rss_mb}MB')
PYEOF
