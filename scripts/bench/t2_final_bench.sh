...
METRICS_OUT="${RMLX_ROOT}/metrics/perf-iter/t2_final.jsonl"
REPORT_LOG="${RMLX_ROOT}/logs/t2_final_bench_$(date -u +%Y%m%d-%H%M%S).log"
...
log() { echo "[t2_final] $*" | tee -a "${REPORT_LOG}" >&2; }
...
_CR_PAYLOAD_TMP="/tmp/rmlx_t2_payload_$$.json"
_CR_RESP_TMP="/tmp/rmlx_t2_resp_$$.json"
...
bench_cell() {
    ...
    SERVE_LOG="${RMLX_ROOT}/logs/t2_final_cell${idx}_${run_id}.log"
