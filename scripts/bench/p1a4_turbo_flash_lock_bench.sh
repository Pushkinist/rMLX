#!/usr/bin/env bash
# P1.A.4 — TurboFlash LOCK-ON bench.
# Qwen3.6-35B-A3B-8bit k8v4 × {16K, 32K, 64K, 128K} ×
#     {OFF=baseline, ON=TurboFlash P1.A.2, ON_LOCK=TurboFlash + RMLX_TURBO_FLASH_LOCK=1}.
...
#
# Usage:  ./scripts/bench/p1a4_turbo_flash_lock_bench.sh
# Output: decode TPS to stdout + summary table at end.
...
LOG_DIR="${RMLX_ROOT}/logs/p1a4_turbo_flash_lock_${RUN_TS}"
mkdir -p "${LOG_DIR}"

log() { echo "[p1a4-bench] $*" >&2; }
...
run_one_request() {
    local prompt_file="$1"
    local payload="/tmp/p1a4_payload_$$.json"
    local resp="/tmp/p1a4_resp_$$.json"
...
run_cell() {
    local ctx="$1"
    local label="$2"
    local mode="$3"   # OFF / ON / ON_LOCK
    local prompt="${RMLX_ROOT}/prompts/longctx_${label}.json"

    log "=== Cell ${label} TurboFlash=${mode} ==="
    preflight

    SERVE_LOG="${LOG_DIR}/serve_${label}_${mode}.log"
    log "Starting server (kv-quant=k8v4, max-ctx=${ctx})..."
    case "${mode}" in
        OFF)
            RMLX_TURBO_FLASH=0 "${RMLX_BIN}" serve \
                --model "${MODEL_PATH}" \
            ...
            ;;
        ON)
            RMLX_TURBO_FLASH=1 "${RMLX_BIN}" serve \
                --model "${MODEL_PATH}" \
                --port "${PORT}" --host 127.0.0.1 \
                --device gpu --kv-quant k8v4 --max-ctx "${ctx}" \
                > "${SERVE_LOG}" 2>&1 &
            ;;
        ON_LOCK)
            RMLX_TURBO_FLASH=1 RMLX_TURBO_FLASH_LOCK=1 "${RMLX_BIN}" serve \
                --model "${MODEL_PATH}" \
                --port "${PORT}" --host 127.0.0.1 \
                --device gpu --kv-quant k8v4 --max-ctx "${ctx}" \
                > "${SERVE_LOG}" 2>&1 &
    ...
    SERVER_PID=$!
    if ! wait_health; then
        log "ERROR: ${label} ${mode} health failed"
...
for cell in "${CELLS[@]}"; do
    ...
    tps_off="$(run_cell "${ctx}" "${label}" OFF)"
    sleep 60
    tps_on="$(run_cell "${ctx}" "${label}" ON)"
    sleep 60
    tps_lock="$(run_cell "${ctx}" "${label}" ON_LOCK)"
    delta_on="$(python3 -c "
off=${tps_off}; on=${tps_on}
print(f'{(on - off) / off * 100:+.1f}%' if off>0 else 'n/a')
")"
    delta_lock="$(python3 -c "
off=${tps_off}; lk=${tps_lock}
print(f'{(lk - off) / off * 100:+.1f}%' if off>0 else 'n/a')
")"
    delta_vs_on="$(python3 -c "
on=${tps_on}; lk=${tps_lock}
print(f'{(lk - on) / on * 100:+.1f}%' if on>0 else 'n/a')
")"
    results+=("${label}|${tps_off}|${tps_on}|${tps_lock}|${delta_on}|${delta_lock}|${delta_vs_on}")
    sleep 60
done
...
echo ""
echo "================================================================="
echo "P1.A.4 TurboFlash LOCK bench — Qwen3.6-35B-A3B-8bit k8v4"
echo "git_sha=${GIT_SHA}, ts=${RUN_TS}"
echo "================================================================="
printf "%-6s | %-9s | %-9s | %-11s | %-9s | %-11s | %-12s\n" \
    "ctx" "OFF TPS" "P1.A.2 ON" "P1.A.4 LOCK" "ON vs OFF" "LOCK vs OFF" "LOCK vs ON"
echo "-------+-----------+-----------+-------------+-----------+-------------+-------------"
for r in "${results[@]}"; do
    ...
    on="$(echo "${r}" | cut -d'|' -f3)"
    lk="$(echo "${r}" | cut -d'|' -f4)"
    d_on="$(echo "${r}" | cut -d'|' -f5)"
    d_lk="$(echo "${r}" | cut -d'|' -f6)"
    d_lk_on="$(echo "${r}" | cut -d'|' -f7)"
    printf "%-6s | %-9s | %-9s | %-11s | %-9s | %-11s | %-12s\n" \
        "${label}" "${off}" "${on}" "${lk}" "${d_on}" "${d_lk}" "${d_lk_on}"
done
echo "================================================================="
