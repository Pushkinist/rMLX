#!/usr/bin/env bash
# VG.2 NIAH surrogate — TurboFlash LOCK-ON correctness gate.
# Compares RMLX_TURBO_FLASH=0 (baseline) vs RMLX_TURBO_FLASH=1+RMLX_TURBO_FLASH_LOCK=1
# on Qwen3.6-35B-A3B-8bit k8v4 @ 32K context. Greedy first-32-token identity.
#
# Exits 0 if identity passes (lock-on path is correct on this machine), 1 on
# divergence (kernel reads stale bf16 buffer — STOP, audit reads).
...
_PAY="/tmp/rmlx_vg2tflock_payload_$$.json"
_RES="/tmp/rmlx_vg2tflock_resp_$$.json"
...
run_one() {
    local mode="$1"   # 'OFF' or 'ON_LOCK'
    local model_id; model_id="$(basename "${QWEN_PATH}")"
    local serve_log="/tmp/vg2_tflock_${model_id}_${mode}.log"
    ...
    local pid=$!
    wait_health
    log "Getting first-32 tokens [${mode}]..."
    local tok
...
log "=== Qwen3.6-35B-A3B-8bit k8v4 32K — TurboFlash LOCK VG.2 ==="

tok_off="$(run_one OFF)"
sleep 60
tok_on="$(run_one ON_LOCK)"

log "OFF:     ${tok_off}"
log "ON_LOCK: ${tok_on}"

if [[ "${tok_off}" == "${tok_on}" ]]; then
    log "PASS: first-32 tokens identical. Lock-on path is correct."
    exit 0
else
    log "FAIL: tokens DIVERGE. Kernel reads stale bf16 buffer — STOP, audit reads."
    exit 1
fi
