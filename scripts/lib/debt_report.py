...
_FN_SIG = re.compile(
    r"^[ \t]*(?:pub(?:\([\w:]+\))?\s+)?(?:async\s+)?(?:unsafe\s+)?fn\s+"
...
def find_matching_brace(text: str, start: int) -> tuple[int, int] | None:
    """Return (body_start, body_end) of the first balanced ``{ ... }`` at or
    after ``start``, skipping over string/char literals and comments so a
    brace inside one of those does not desync the depth count."""
    i, n = start, len(text)
    depth = 0
    body_start = None
    while i < n:
        ...
        if c == '"':
            ...
            continue
        if c == "'":
            # Char literal ('a', '\n') closes within 2-3 chars; a lifetime
            # ('a) does not. Only consume it as a literal if it does close.
            j = i + 2 if (i + 1 < n and text[i + 1] == "\\") else i + 1
            if j < n and text[j] == "'":
                i = j + 1
            else:
                i += 1
...
def extract_fns(text: str) -> dict[str, str]:
    ...
    for m in _FN_SIG.finditer(text):
        span = find_matching_brace(text, m.end())
        if span is None:
            continue
        start, end = span
        fns[m.group("name")] = text[start:end]
    return fns
...
def report_sibling_similarity(root: Path, lines: list[str]) -> None:
    ...
    # Named group: the six speculative round-loop drivers, wherever each one
    # is defined under SPEC_DIR. Always printed, not gated on the threshold —
    # this is the standing violation the twin rule names, not a candidate.
    lines.append("")
    lines.append(f"--- round-loop drivers ({SPEC_DIR}) ---")
    driver_bodies: dict[str, str] = {}
    driver_files: dict[str, str] = {}
    for f in rust_files(root, SPEC_DIR, include_tests=False):
        text = f.read_text(errors="ignore")
        for name, body in extract_fns(text).items():
            if name in ROUND_LOOP_DRIVERS and name not in driver_bodies:
                driver_bodies[name] = body
                driver_files[name] = str(f.relative_to(root))
    for name in ROUND_LOOP_DRIVERS:
        where = driver_files.get(name, "NOT FOUND")
        lines.append(f"  {name:<24} {where}")
    lines.append("  pairwise similarity:")
    for a, b in itertools.combinations(ROUND_LOOP_DRIVERS, 2):
        if a in driver_bodies and b in driver_bodies:
            pct = similarity(driver_bodies[a], driver_bodies[b]) * 100
            lines.append(f"    {a} <-> {b}: {pct:.1f}%")
        else:
            lines.append(f"    {a} <-> {b}: not found")
    ...
    for a, b in file_pairs(root):
        ...
        reported = True
        lines.append(
            f"  {a.relative_to(root)} <-> {b.relative_to(root)}: {pct:.1f}% shared"
        )
        a_fns = {normalize(k): (k, v) for k, v in extract_fns(a_text).items()}
        b_fns = {normalize(k): (k, v) for k, v in extract_fns(b_text).items()}
        for norm_name in sorted(set(a_fns) & set(b_fns)):
            fa_name, fa_body = a_fns[norm_name]
            fb_name, fb_body = b_fns[norm_name]
            fpct = similarity(fa_body, fb_body) * 100
            if fpct / 100 >= SIM_THRESHOLD:
                label = fa_name if fa_name == fb_name else f"{fa_name}/{fb_name}"
                lines.append(f"    fn {label}: {fpct:.1f}% shared")
    if not reported:
...
def report_debt_counters(root: Path, lines: list[str]) -> None:
    ...
    lines.append(f"#[allow(...)] sites (non-test source): {allow_sites}")
    ...
    for f in rust_files(root, WORKSPACE_SOURCE_DIR, include_tests=True):
        for line in f.read_text(errors="ignore").splitlines():
            ...
            if stripped.startswith("//") and DEBT_COMMENT_RE.search(stripped):
                debt_comments += 1
    lines.append(
        "debt-marker comments (inert|dormant|deferred|kept for|"
        f"future-reference|no longer): {debt_comments}"
    )
    ...
    for f in rust_files(root, WORKSPACE_SOURCE_DIR, include_tests=False):
        text = f.read_text(errors="ignore")
        if text.count("\n") + 1 > LOC_THRESHOLD and "LOC-exempt" not in text:
            oversized += 1
    lines.append(f"files >{LOC_THRESHOLD} LOC without a LOC-exempt marker: {oversized}")
...
# ---- section 3: add/remove ratio since the last tag ------------------------
...
def format_ratio(ins: int, dels: int) -> str:
    if dels == 0:
...
def report_add_remove_ratio(root: Path, since: str | None, lines: list[str]) -> None:
    ...
    lines.append("")
    if ref is None:
        lines.append("=== add/remove ratio: no tag found, showing full history ===")
        rev_range = "HEAD"
    else:
        lines.append(f"=== add/remove ratio since {ref} ===")
        rev_range = f"{ref}..HEAD"
    ins, dels = shortstat_totals(root, rev_range, ["crates"])
    lines.append(f"source (crates/): {format_ratio(ins, dels)}")
    ...
    ins, dels = shortstat_totals(root, rev_range, ["docs", "*.md"])
    lines.append(f"docs (docs/, *.md): {format_ratio(ins, dels)}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--since",
        default=None,
        help="git ref for the add/remove ratio (default: last tag)",
    )
