...
#   GPU-touching.
#
#   Fail-closed companion: a `macro_rules!` body that contains `#[test]` but
#   from which NO test item could be extracted is itself an error (`U` below).
#   That is the case where a name is assembled rather than written — a `paste!`
#   / `concat_idents!` body, or a `#[test]` on the same line as its `fn`. The
#   gate cannot classify it, and reporting OK over a test it could not read is
#   the exact defect this section exists to close.
#
...
#     fail-closed `U` case above catches it; at top level it does not, and
#     rustfmt's own layout is what keeps the shape out of the tree.
#
...
read -r -d '' AWK_DETECT <<'AWK' || true
    ...
    !in_fn && !in_macro && /^[[:space:]]*macro_rules![[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*\{/ {
        in_macro = 1
        macro_name = $0
    ...
    !in_fn && /^[[:space:]]*(pub[^ ]* )?(async )?fn [$a-zA-Z0-9_]/ {
        ...
        if (substr(name, 1, 1) == "$") {
            gen_of[G] = (macro_name != "" ? macro_name : "<macro>")
    ...
    END {
        # A macro body left open by the last file still gets its readability
        # check — otherwise an unterminated body is a free pass.
        flush_macro()
...
if [ -n "$unreadable" ]; then
    ...
    echo "fn name). A name assembled by paste!/concat_idents!, or a #[test] sharing" >&2
    echo "its fn's line, is invisible — and an invisible GPU test is exactly what" >&2
