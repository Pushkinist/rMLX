...
/// *one-dtype* discipline, but on an fp16 checkpoint it picks **fp16** (float16
/// params, logits and KV cache). rMLX picks bf16 instead, which is 3 mantissa
/// bits coarser than both the checkpoint and the reference.
///
/// That is a deliberate trade: bf16 is the dtype the rest of this engine's
/// kernels and KV codecs are built around. The cost is real — it moves tokens
/// at near-tie logits. Do not describe this cast as matching the reference.
