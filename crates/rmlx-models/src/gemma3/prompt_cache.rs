...
//! - `impl HydratedEntry for Gemma3Entry`: what an SSD-restored block becomes
//!   here (pure-attention: the block's `lin_caches` are discarded, and the SWA
//!   rotating-ring layers come back payload-less). The probe itself is the
//!   blanket `SsdHydrate<E> for SsdHydrator` in `rmlx-kv-ssd`, and the SSD
//!   spill path is the blanket `SpillSink<E> for SsdSpiller` in
//!   `crate::prompt_cache`.
//!
...
/// What an SSD-restored block becomes as a `Gemma3Entry`.
///
/// Pure-attention arch: the block carries `kv_caches` only, and its
/// `lin_caches` are discarded. The SWA rotating-ring layers come back
/// payload-less (the ring is not serialised). The matched block-aligned prefix
/// token IDs become the entry's `prompt_token_ids`, and the runtime `kv_quant`
/// is recorded. `first_id` / `first_piece` are sentinels (the SSD block stores
/// no first decode token), so the entry is flagged `is_ssd_hydrated = true`;
/// the consume engine excludes it from the Exact fast path and the generate
/// loop recomputes the real first token via re-prefill.
impl HydratedEntry for Gemma3Entry {
    const SHARES_KV: bool = crate::gemma3::SHARES_KV_ACROSS_LAYERS;
