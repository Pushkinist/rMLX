...
//! - `impl HydratedEntry for Qwen3VlMoeEntry`: what an SSD-restored block
//!   becomes here (pure-attention: the block's `lin_caches` are discarded). The
//!   probe itself is the blanket `SsdHydrate<E> for SsdHydrator` in
//!   `rmlx-kv-ssd`, and the SSD spill path is the blanket
//!   `SpillSink<E> for SsdSpiller` in `crate::prompt_cache`, which spills
//!   `kv_caches()` only for a pure-attention entry (`lin_caches()` is `&[]`).
//!
...
/// What an SSD-restored block becomes as a `Qwen3VlMoeEntry`.
///
/// Plain-attention text decoder: the block carries `kv_caches` only, and its
...
impl HydratedEntry for Qwen3VlMoeEntry {
    const SHARES_KV: bool = crate::qwen3_vl_moe::SHARES_KV_ACROSS_LAYERS;
...
pub(crate) static PROMPT_CACHE: ArchPromptCache<Qwen3VlMoeEntry> = ArchPromptCache::new(
