...
fn hydrated_truncate_keeps_prefix_and_correction(quant: KvQuant, kv_h: i32) {
    ...
    // Premise: a hydrated cache really does run the codec arm rather than the
    // bf16 decode shortcut. Without this the rest of the test is vacuous.
    assert!(
        cache.decode_fp16_kv().is_none(),
        "{label}: a hydrated cache must carry no bf16 decode seed, or the codec \
         store is never read and this test proves nothing"
    );

    // Reference decode of the untruncated store — the oracle for the prefix.
    ...
    assert_eq!(
        ...
        "{label}: store must decode to exactly the retained prefix plus the correction"
    );

    // 1. The retained prefix is bit-identical to the same positions of the
    //    pre-truncate decode. Head-major layout, so index per (head, position).
    for h in 0..kv_h {
        for s in 0..keep {
            for dd in 0..head_dim {
                let got = after_v[((h * total + s) * head_dim + dd) as usize];
                let want = full_v[((h * prefill + s) * head_dim + dd) as usize];
                assert!(
                    (got - want).abs() < f32::EPSILON,
                    "{label}: retained prefix changed at h={h} s={s} d={dd}: {got} vs {want}"
                );
            }
        }
    }
    ...
    for h in 0..kv_h {
        for s in 0..corr {
            for dd in 0..head_dim {
                let got = after_v[((h * total + keep + s) * head_dim + dd) as usize];
                let want = corr_v_raw[((h * corr + s) * head_dim + dd) as usize];
                let discarded = full_v[((h * prefill + keep + s) * head_dim + dd) as usize];
    ...
    assert!(
        corr_err < 0.2,
        "{label}: the appended correction is missing — the tail differs from its \
         own source by {corr_err} (a pre-fix store returns the discarded tokens here)"
    );
    assert!(
        stale_err > corr_err,
        "{label}: premise — the discarded tokens must be distinguishable from the \
         correction, else this assertion cannot fail (corr_err={corr_err}, \
         stale_err={stale_err})"
    );
}
