...
#[allow(
    ...
    reason = "named template args make each MSL placeholder traceable to its Rust source value; inlining would interleave Rust expressions with multi-line MSL text and harm readability"
)]
fn build_qk_kernel_source(bits: u8) -> String {
    // 4-bit: 8 values per u32 word (8 * 4 = 32 bits).
    // 3-bit: 10 values per u32 word (10 * 3 = 30 bits, 2 wasted).
    let (vals_per_word, decode_idx_expr): (u32, &str) = match bits {
        4 => (
            8,
            "(codes[code_word_abs] >> ((elem_in_group & 7u) * 4u)) & 0xFu",
        ),
        3 => (
            10,
            "(codes[code_word_abs] >> ((elem_in_group % 10u) * 3u)) & 0x7u",
        ),
    ...
    // Both 3-bit and 4-bit pack 32 elements into 4 u32 words (GROUP_SIZE / 8).
    ...
    // Codes (bits = {bits_u}): word_in_group = elem_in_group / vals_per_word.
