//! Parity tests for the fused TurboSym3 K-side QK kernel.
...
#[test]
fn turbo_k3_fused_qk_dispatch_count_starts_at_zero() {
    // Counter is process-global; verify it is readable without panicking.
    let _ = turbo_k3_fused_qk_dispatch_count();
}

#[test]
fn turbo_k3_fused_qk_rejects_unsupported_head_dim() {
    ...
    let err = turbo_k3_fused_qk_sdpa(
        &dummy_q,
        &dummy_codes,
        &dummy_scales,
        None,
    ...
    .expect_err("turbo_k3_fused_qk_sdpa should reject head_dim=64");
...
#[test]
#[ignore = "GPU Metal context — run in isolation: cargo test turbo_k3_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn turbo_k3_fused_qk_single_tile_head_dim_128() {
    // Single-tile config:
    ...
    let k_data = lcg_data(k_n, 0x7B30_5ED1_u64);
    ...
    let q_data = lcg_data(q_n, 0x7B30_5ED2_u64);
    let q_arr = make_f32_array(&q_data, &q_shape);

    // Quant + dequant K via turbo3 GPU codec; the dequantized K is the
    // reference the fused kernel must match.
    let (codes, scales) = turbo_quantize_v3_gpu(&k_arr, Device::Gpu).expect("turbo3 quantize");
    let k_dequant = turbo_dequantize_v3_gpu(&codes, &scales, &k_shape, Dtype::F32, Device::Gpu)
        .expect("turbo3 dequant");
    let k_dequant_vec = array_to_f32(&k_dequant);

    let fused = turbo_k3_fused_qk_sdpa(
        &q_arr,
        &codes,
        &scales,
        None,
    ...
    assert!(
        max_err < 1e-4_f32,
        "turbo_k3_fused_qk single-tile head_dim=128: max abs error {max_err:.6} > 1e-4"
    );
}
...
#[ignore = "GPU Metal context — run in isolation: cargo test turbo_k3_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn turbo_k3_fused_qk_multi_tile_head_dim_128_kv_seq_256() {
    // Multi-tile config:
    ...
    let k_shape = [b, kv_h, kv_seq, head_dim];
    let k_n: usize = k_shape.iter().map(|&d| d as usize).product();
    let k_data = lcg_data(k_n, 0x7B30_4321_u64);
    let k_arr = make_f32_array(&k_data, &k_shape);

    let q_shape = [b, n_q_heads, 1, head_dim];
    let q_n: usize = q_shape.iter().map(|&d| d as usize).product();
    let q_data = lcg_data(q_n, 0x7B30_8765_u64);
    let q_arr = make_f32_array(&q_data, &q_shape);
    ...
    assert!(
        max_err < 1e-4_f32,
        "turbo_k3_fused_qk multi-tile (kv_seq=256) head_dim=128: max abs error {max_err:.6} > 1e-4"
    );
}
...
#[ignore = "GPU Metal context — run in isolation: cargo test turbo_k3_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn turbo_k3_fused_qk_dispatch_count_increments() {
    if skip_if_no_gpu_env() {
        return;
    }
    let before = turbo_k3_fused_qk_dispatch_count();
    ...
    let k_shape = [b, kv_h, kv_seq, head_dim];
    let k_n: usize = k_shape.iter().map(|&d| d as usize).product();
    let k_data = lcg_data(k_n, 0x7B30_0001_u64);
    let k_arr = make_f32_array(&k_data, &k_shape);

    let q_shape = [b, kv_h * heads_per_kv, 1, head_dim];
    let q_n: usize = q_shape.iter().map(|&d| d as usize).product();
    let q_data = lcg_data(q_n, 0x7B30_0002_u64);
    let q_arr = make_f32_array(&q_data, &q_shape);

    let (codes, scales) = turbo_quantize_v3_gpu(&k_arr, Device::Gpu).expect("turbo3 quantize");
    let _ = turbo_k3_fused_qk_sdpa(
        &q_arr,
        &codes,
        &scales,
        None,
    ...
    let after = turbo_k3_fused_qk_dispatch_count();
    assert!(
        after > before,
