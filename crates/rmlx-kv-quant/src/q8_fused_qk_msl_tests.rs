//! Parity tests for the fused q8 K-side QK MSL kernel.
...
#[test]
fn q8_fused_qk_dispatch_count_starts_at_zero() {
    // Counter is process-global; verify it is readable without panicking.
    let _ = q8_fused_qk_dispatch_count();
}

#[test]
fn q8_fused_qk_rejects_unsupported_head_dim() {
    // head_dim=64 is not supported (kernel uses fixed register sizes
    // for 128/256 only).  The dispatcher must reject before any GPU work,
    // so dummy 1-element arrays are fine.
    ...
    .expect_err("q8_fused_qk_sdpa should reject head_dim=64");
...
#[test]
#[ignore = "GPU Metal context — run in isolation: cargo test q8_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn q8_fused_qk_gqa_single_tile_head_dim_128() {
    // GQA single-tile config:
    ...
    let k_data = lcg_data(k_n, 0xCAFE_5ED1_u64);
    ...
    let q_data = lcg_data(q_n, 0xBEEF_5ED2_u64);
    let q_arr = make_f32_array(&q_data, &q_shape);

    // Quant + dequant K via q8 GPU codec; the dequantized K is the reference.
    let (codes, scales) = q8_quantize_gpu(&k_arr, Device::Gpu).expect("q8 quantize");
    let k_dequant =
        q8_dequantize_gpu(&codes, &scales, &k_shape, Dtype::F32, Device::Gpu).expect("q8 dequant");
    let k_dequant_vec = array_to_f32(&k_dequant);

    let fused = q8_fused_qk_sdpa(
        &q_arr,
        &codes,
        &scales,
        None,
    ...
    assert!(
        max_err < 1e-4_f32,
        "q8_fused_qk single-tile head_dim=128: max abs error {max_err:.6} > 1e-4"
    );
}
...
#[ignore = "GPU Metal context — run in isolation: cargo test q8_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn q8_fused_qk_gqa_multi_tile_head_dim_128_kv_seq_256() {
    // GQA multi-tile config:
    ...
    assert!(
        max_err < 1e-4_f32,
        "q8_fused_qk multi-tile (kv_seq=256) head_dim=128: max abs error {max_err:.6} > 1e-4"
    );
}
...
#[ignore = "GPU Metal context — run in isolation: cargo test q8_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn q8_fused_qk_head_dim_256() {
    // head_dim=256 path (Gemma4-26B class): two q8 groups per head_dim.
    if skip_if_no_gpu_env() {
        return;
    }
    let b: i32 = 1;
    ...
    assert!(
        max_err < 1e-4_f32,
        "q8_fused_qk head_dim=256: max abs error {max_err:.6} > 1e-4"
    );
}
...
#[ignore = "GPU Metal context — run in isolation: cargo test q8_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn q8_fused_qk_with_additive_mask() {
    ...
    let k_shape = [b, kv_h, kv_seq, head_dim];
    let k_n: usize = k_shape.iter().map(|&d| d as usize).product();
    let k_data = lcg_data(k_n, 0xDEAD_0001_u64);
    let k_arr = make_f32_array(&k_data, &k_shape);

    let q_shape = [b, n_q_heads, 1, head_dim];
    let q_n: usize = q_shape.iter().map(|&d| d as usize).product();
    let q_data = lcg_data(q_n, 0xDEAD_0002_u64);
    let q_arr = make_f32_array(&q_data, &q_shape);
    ...
    .expect("q8_fused_qk_sdpa with mask");
    ...
    assert!(
        max_err < 1e-3_f32,
        "q8_fused_qk with mask: max abs error {max_err:.6} > 1e-3"
    );
}
...
#[ignore = "GPU Metal context — run in isolation: cargo test q8_fused_qk -- --ignored --test-threads=1"]
#[allow(clippy::expect_used, reason = "test invariants documented")]
fn q8_fused_qk_dispatch_count_increments() {
    if skip_if_no_gpu_env() {
        return;
    }
    let before = q8_fused_qk_dispatch_count();
    ...
    let k_shape = [b, kv_h, kv_seq, head_dim];
    let k_n: usize = k_shape.iter().map(|&d| d as usize).product();
    let k_data = lcg_data(k_n, 0xABCD_0001_u64);
    let k_arr = make_f32_array(&k_data, &k_shape);

    let q_shape = [b, kv_h * heads_per_kv, 1, head_dim];
    let q_n: usize = q_shape.iter().map(|&d| d as usize).product();
    let q_data = lcg_data(q_n, 0xABCD_0002_u64);
    let q_arr = make_f32_array(&q_data, &q_shape);

    let (codes, scales) = q8_quantize_gpu(&k_arr, Device::Gpu).expect("q8 quantize");
    let _ = q8_fused_qk_sdpa(
        &q_arr,
        &codes,
        &scales,
        None,
    ...
    let after = q8_fused_qk_dispatch_count();
    assert!(
        after > before,
