    ...
    const uint LOG2_D_C      = 9u;
    const uint GROUPS_ROW    = 8u;
    const uint WORDS_ROW     = 128u;
    const float INV_SQRT_D   = 1.0f / sqrt((float)D_C);

    threadgroup float buf[512];
    ...
    threadgroup float grp_max[8];
    threadgroup float grp_min[8];
