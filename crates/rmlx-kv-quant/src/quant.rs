...
impl KvQuant {
    ...
    /// One representative instance of every `KvQuant` variant.
    ///
    /// Not `#[cfg(test)]`: this is the enumeration cross-crate drift guards
    /// consume — e.g. the `rmlx-metrics` kv_quant allow-list mirror test in
    /// `rmlx-models` (the one crate that depends on both this crate and
    /// `rmlx-metrics`; `rmlx-metrics` itself must not depend on
    /// `rmlx-kv-quant`, so it cannot enumerate variants directly). A
    /// `#[cfg(test)]` item here would not be visible to that downstream
    /// crate's test binary.
    ///
    /// Payload-bearing variants use one illustrative parameter set — the
    /// full parameter matrix is exercised separately by the round-trip test
    /// in `quant_tests.rs`. There is no compiler-enforced exhaustiveness
    /// here (a `Vec` literal is not a `match`), so this list is a maintained
    /// mirror of the enum, same as the round-trip test above it — keep both
    /// in sync when a variant is added or removed.
    #[must_use]
    pub fn one_of_each_variant() -> Vec<KvQuant> {
        vec![
            KvQuant::None,
            KvQuant::K8V4,
            KvQuant::K8V8,
