...
fn migrate_one_csv_row_inserts_multiple_metrics() {
    ...
    // The row carries 7 parseable metric columns. Two of them are CBB's "not
    // measured" placeholders and neither may become an observation:
    // `task_pass_at_1=0.0` (dropped at the parse site — the value alone is a valid
    // score, only the column convention says otherwise) and `peak_rss_mb=0.0`
    // (dropped by the `docs/METRICS_SCHEMA.md` §4.1 bounds — a live process has
    // RSS). Exact counts, not `>=`: a `>=` here passes whether 0 or 3 entries were
...
/// `docs/METRICS_SCHEMA.md` §4.1 bounds cannot catch it — `0.0` pass@1 is a
/// legitimate score for a model that failed every task — so the parse site
/// must, or the placeholder wins every all-zero partition in `bests` exactly
/// like the rate zeros do.
#[test]
