...
| `make debt-report` | Advisory (non-failing): sibling-file/fn similarity ("twins") in `crates/rmlx-kv-quant` and `crates/rmlx-models` — printing the six speculative round-loop drivers (`mtp_generate`, `dflash_generate`, `dflash2_generate`, `eagle3_generate`, `mtp_assistant_generate`, `spec_generate_greedy`) as a named group regardless of threshold, since that pairing is the standing violation the twin rule names — plus `#[allow(` and debt-marker-comment counts, the add/remove line ratio since the last tag, and `docs/*.md` files over 200 KB. Also runs at the end of `make ci` (advisory, non-blocking). |
| `make debt-report-selftest` | CI gate (in `make ci`): the above over synthetic fixtures — a planted twin pair is reported, a same-naming-shape pair with unrelated bodies is not, the round-loop group names all six drivers, and a synthetic two-commit git repo proves the add/remove ratio is computed correctly. Asserts what each case found, not just that the tool ran. |
