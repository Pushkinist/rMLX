...
| `make check-doc-refs` | Every reference into `docs/` that resolved at the merge-base with `origin/main` still resolves to the same target: links, anchors, `§` sections, quoted phrases, line citations, and the documentation-map rows. Run it on every doc edit. `make check-doc-refs-selftest` is its recall test (in `make ci`). |
