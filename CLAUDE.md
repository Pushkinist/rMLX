...
| [`docs/KV_QUANT.md`](docs/KV_QUANT.md) | KV quantization contract: public API, import paths, storage summary, Metal-vs-CPU hot path, CLI flags and presets, the auto default, memory and bit rates, the break-even condition and each codec's disposition |
| [`docs/KV_LAYER_POLICY.md`](docs/KV_LAYER_POLICY.md) | Which KV codec each layer gets: per-layer net-benefit decision, bf16 at `--kv-quant none`, byte accounting, hot-swap, dispatch axis, layer-adaptive overrides, Qwen MoE low-bit K rejection |
...
| [`docs/KV_FUSED_KERNELS.md`](docs/KV_FUSED_KERNELS.md) | Fused KV decode kernels: fused-QK, fused flash-decode over rotor / iso / PlanarK, fused-QK head-major K storage, sparse attention |
...
| `make check-kv-codec-disposition` | CI gate (in `make ci`): the `--kv-quant` / `--kv-bits` help and the INERT banners agree with each codec's runtime disposition, derived from `ALL_KV_QUANTS` + `decode_reads_packed_store` / `feeds_bf16_{k,v}_at_decode`. The banners are read from a fixed list of docs (`BANNER_DOCS`); a banner in any other doc fails, a listed doc that is missing is exit 2, and each inert codec is named in exactly one banner. |
| `make check-kv-codec-disposition-fixtures` | CI gate (in `make ci`): recall test for the above, 23 synthetic scan roots (one edit each), each asserting which rule fired and exit 2 vs exit 1. |
