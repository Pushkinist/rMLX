...
| `make spec-bench-published-selftest` | CI gate (in `make ci`): mutation check for `scripts/spec_bench_published.sh` against a stub server over a shrunken copy of the checked-in sample sets, 70 cases, each asserting a literal exit code and, for every refusal, the reason. No GPU, no model, no DB. |
...
| `make check-spec-charge` | CI gate (in `make ci`): each speculative round loop names one phase-charge decision and names it everywhere the decision is read — the `charge` argument of every `rollback_round_caches` call inside the loop and the `charged:` field of every `RoundPhases` event and of the `RoundTotals` it hands the one recorder are the same token, and no fn outside the population writes the field at all — the recorder carries it across as a destructured `charged,`, and `crates/rmlx-models/src/speculative/round_common_tests.rs` is what reads that it arrives — and the census over the seven loops is exactly `charge_phases` three times and `false` four. A loop that charges its phases and records that it did not re-attributes its own work to the drafter with every token and every count identical, and no other observable in the tree can see it: the equivalence pairs read the answer, the accept counters read the aggregate, and the per-round stream reads `charged` as `false` on both sides by construction, because a capture that enabled the switch would be measuring a different, slower run. Fewer loops than the tree ships, or a call whose arguments the scan cannot read back, is exit 2. |
| `make check-spec-charge-fixtures` | CI gate (in `make ci`): recall test for the above, 27 synthetic scan roots carrying the tree's own census, each asserting the reason as well as exit 1 vs exit 2 — including the two shapes that drop a loop out of the derived population, which must read as a lost loop and not as a census that moved, and the shared seed emit, which carries a driver's signature and is kept out by naming no totals of its own. |
