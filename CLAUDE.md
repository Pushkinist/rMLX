...
| `make check-published-samples` | CI gate (in `make ci` and hosted CI): the checked-in published-protocol sample sets under `prompts/published/` re-derive from their manifest — file digests and byte lengths, the draw from its seed over the recorded pool, each user message from its template and verbatim upstream record, each body digest from its messages. |
| `make check-published-samples-fixtures` | CI gate (in `make ci` and hosted CI): recall test for the above, 18 synthetic sample-set roots (one edit each), each asserting the reason as well as exit 1 vs exit 2. |
