...
| `make check-spec-sampling` | CI gate (in `make ci`): every speculative generation driver — a `pub fn` taking `step_fn: &mut dyn FnMut(&ProbeStep)` — is handed the request's sampler and reads it, and every drafter arm of the server's dispatch passes it. A loop that takes no sampler decodes greedily, does not warn, and returns fluent text, so the caller's temperature is lost with nothing anywhere saying so. This gate cannot tell a right distribution from a wrong one; `crates/rmlx-models/tests/spec_sampled_distribution.rs` does that, on one pair under `make gpu-test`. A scan matching no driver is exit 2, not a pass. |
| `make check-spec-sampling-fixtures` | CI gate (in `make ci`): recall test for the above, 10 synthetic scan roots, each asserting the reason as well as exit 1 vs exit 2. |
