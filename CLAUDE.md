...
| [`docs/SPEC_ROUND_SKELETON.md`](docs/SPEC_ROUND_SKELETON.md) | **Migrating**: one round loop and a drafter interface for the seven speculative loops; the drafters move onto it one per chunk, and the "What chunk N landed" sections say which are on it — what it cannot express, the migration order, the oracle and the mutation list |
