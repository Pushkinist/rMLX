...
| `make gpu-test` | Run the GPU/Metal `#[ignore]` tests, `--test-threads=1` (`CRATE=` / `FILTER=` narrow). Needs exclusive machine access. Under Metal shader validation: the hits it observes are diffed against the pinned census `scripts/gpu_validation_census.txt`, an exact match passes and prints what it accepted, any deviation fails naming the delta. Part of `make ci-perf`, not `make ci`. |
...
| `make ci-perf` | `test-perf` under `release-perf` + the serialized GPU/Metal suite. Requires an idle GPU, and a snapshot root if the census pin's entries are to be checked in full. Run before merging perf-sensitive or codec-layer changes (~21 min). |
