...
| `make debt-report` | Advisory (non-failing): sibling-file/fn similarity ("twins") in `crates/rmlx-kv-quant` and `crates/rmlx-models` — printing the speculative round-loop drivers as a named group regardless of threshold, since that pairing was the standing violation the twin rule names; the loops have since collapsed onto one body, and narrowing the group to the charge gate's population is the open item. The group is *discovered*, not a literal name list: a driver is any fn under `crates/rmlx-models/src/speculative/` whose signature carries `step_fn: &mut dyn FnMut(&ProbeStep)`, the same rule `scripts/check_spec_sampling.sh` uses — generalised to any visibility, which is what surfaces the private cached loops a `pub fn`-only list misses. Four debt counters (`#[allow(` sites, debt-marker comments, `check-*` Make targets, oversized files without `LOC-exempt`, each stating its own file population), the churn (summed over commits, not a net diff) since the last tag, and `docs/*.md` files over 200 KB. Also runs at the end of `make ci` and in hosted CI (advisory, non-blocking). |
| `make debt-report-selftest` | CI gate (in `make ci` and hosted CI): the above over synthetic fixtures — a planted twin pair is reported and a same-naming-shape pair just under the threshold is not, the round-loop group is discovered by signature (not by name — a renamed driver is found under its new name, with the total count unaffected), two named `fn …: NN.N% shared` lines are asserted from the twin pair, all four debt counters are asserted against planted fixtures (one `#[allow(`, one debt comment in source and one in a `_tests.rs` file, a Makefile with two `check-*:` targets, an oversized file and its `LOC-exempt`-marked twin), and a synthetic two-commit, one-tag git repo (plus a top-level `README.md` alongside a `docs/*.md` file) proves the churn figure and its "*.md" pathspec are both exercised. Asserts what each case found, not just that the tool ran. |
