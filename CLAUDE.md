...
| `make canary-ab-host-gate-fixtures` | CI gate (in `make ci`): recall test for that boundary — the quiescence and Metal-exclusivity gates still fire on a shimmed hostile host, a hostile and a quiet host give `--synthetic-arms` the same verdict, and the flag waives no arm-reading guard. |
