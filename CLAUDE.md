...
| `make spec-bench-published-selftest` | CI gate (in `make ci`): mutation check for `scripts/spec_bench_published.sh` against a stub server over a shrunken copy of the checked-in sample sets, 72 cases, each asserting a literal exit code and, for every refusal, the reason. No GPU, no model, no DB. |
